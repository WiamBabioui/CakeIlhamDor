...
export default function WhatsAppButton({ productName, lang, className = '' }) {
  const isRTL = lang === 'ar'
  const message = isRTL
    ? `مرحباً، أريد طلب: ${productName}`
    : `Bonjour, je voudrais commander : ${productName}`
  ...
  return (
    <a
      ...
      className={`inline-flex items-center justify-center gap-2.5 bg-[#25D366] hover:bg-green-500 text-white font-semibold py-3.5 px-7 rounded-2xl transition-all duration-300 hover:scale-105 hover:shadow-lg ${className}`}
    >
...
export function FloatingWhatsApp({ lang }) {
  const isRTL = lang === 'ar'
  const message = isRTL ? 'مرحباً، أريد الاستفسار عن طلب' : 'Bonjour, je voudrais passer une commande'
  const url = `https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent(message)}`
  return (
    <a
      href={url}
